...
import com.axway.adi.tools.disturb.parsers.contexts.ThreadDumpContext;
    ...
    private void readThreadDumps(Path redoLogFile) {
        ...
        try (BufferedReader reader = new BufferedReader(new FileReader(redoLogFile.toFile()))) {
            ...
            while ((line = reader.readLine()) != null) {
                ...
                // Read header
                if (header) {
                    if (line.startsWith(ThreadDump.THREAD_NAME_HEADER)) {
                        header = false;
                    }
                }
                if (previousLineBlank && line.startsWith(ThreadDump.THREAD_NAME_HEADER)) {
                    inThread = true;
                    addThread(line);
                } else if (inThread) {
                    addStack(line);
